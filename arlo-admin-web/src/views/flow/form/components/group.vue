<template>
    ...
    <div class="left-group-header">
      <div class="left-group-title">
        ...
        <div class="title-actions">
          <el-button
            v-permission="'flow:formCategory:manage'"
  ...
  <el-dialog v-model="groupModel" :title="groupData.id ? '编辑分类' : '添加分类'" width="500" :close-on-click-modal="false">
...
import {
  getFlowFormCategoryTree,
  createFlowFormCategory,
  updateFlowFormCategory,
  deleteFlowFormCategory,
} from '@/api'
...
async function initList(row: any = {}) {
  const res = await getFlowFormCategoryTree()
...
function delGroup(row: any) {
  if (row.formList?.length) {
    ElMessage.warning('请先删除关联表单')
    return
  }
  ElMessageBox.confirm('确定将选择数据删除?', {
    ...
    .then(async () => {
      await deleteFlowFormCategory(row.id)
...
function groupView(row: any) {
  ...
  if (row.id == '-1') {
    const all = { id: '-1', formList: [] as any[] }
    defaultGroupList.value.forEach((item: any) => {
      if (item.formList) all.formList.push(...item.formList)
    })
    emits('event', all)
    return
  }

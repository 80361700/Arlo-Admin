    ...
    <div class="pending-row">
      <aside class="pending-left">
        <div class="left-pane">
          ...
          >
            ...
            >
              <div class="card-top">
                ...
                <div class="card-tags">
                  ...
                  <el-tag size="small" type="warning" effect="plain">待认领</el-tag>
                </div>
              </div>
              <div class="card-node">认领节点：{{ row.nodeName || '-' }}</div>
              <div class="card-meta">
            ...
            <el-empty v-if="!filteredList.length && !listLoading" description="暂无可认领任务" :image-size="72" />
      ...
      <section class="pending-right">
        <div class="right-pane">
            ...
            <div v-loading="detailLoading" class="detail-wrap">
              ...
              <div class="detail-footer">
                <div class="footer-actions">
                  ...
                  <el-button
                    v-permission="'flow:approve:claim'"
                    type="primary"
                    :loading="acting"
                    @click="onClaim"
                  >
          ...
          <el-empty v-else class="right-empty" description="请选择左侧任务查看详情" :image-size="96" />
        </div>
      </section>
    </div>
...
const nodeRunActors = computed(() => {
  ...
  if (d.currentNodeKey) {
    for (const a of d.taskActors || []) {
      ...
      const name = Number(a.actorType) === 2 ? `${a.actorName}(待认领)` : a.actorName
      push(d.currentNodeKey, name, a.actorType)
    }
  }
  return map
})
...
async function loadList(append = false) {
  ...
  try {
    ...
    const res = await getClaimableTasks({
      page: page.value,
...
async function onClaim() {
  const taskId = selected.value?.taskId || detail.value?.taskId
  if (!taskId) return
  acting.value = true
  try {
    await claimApproveTask({ taskId })
    ElMessage.success('认领成功，请到「待审批」处理')

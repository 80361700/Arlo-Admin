    ...
    <div class="pending-row">
        ...
        <div class="left-pane">
          ...
          >
            <div
              ...
              :key="rowKey(row)"
              class="task-card"
              :class="{ active: isSelected(row), unread: row.read === false }"
              @click="selectRow(row)"
            >
              <div class="card-top">
                <div class="card-title" :title="row.processName">
                  <span v-if="row.read === false" class="unread-dot" />
                  {{ row.processName }}
                </div>
                <div class="card-tags">
              ...
              <div class="card-node">抄送节点：{{ row.nodeName || '-' }}</div>
              <div class="card-meta">
            ...
            <el-empty v-if="!filteredList.length && !listLoading" description="暂无抄送" :image-size="72" />
      ...
      <section class="pending-right">
        <div class="right-pane">
          ...
          <el-empty v-else class="right-empty" description="请选择左侧抄送查看详情" :image-size="96" />
        </div>
      </section>
    </div>
...
function rowKey(row: ApproveInstanceListItem) {
  ...
  if (row.hisTaskId) return `h-${row.hisTaskId}-${row.instanceId}`
...
async function loadList(append = false) {
  ...
  try {
    ...
    const res = await getReceivedApprovals({
      page: page.value,
...
async function selectRow(row: ApproveInstanceListItem, opts?: { keepTab?: boolean }) {
  ...
  try {
    ...
    if (row.id && row.read === false) {
      try {
        await markReceivedApprovalRead(row.id)
        const hit = list.value.find((r) => rowKey(r) === rowKey(row))
        if (hit) hit.read = true
        if (selected.value && rowKey(selected.value) === rowKey(row)) {

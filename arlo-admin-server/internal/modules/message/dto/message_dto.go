...
type MessageListQuery struct {
	Title     string `form:"title" example:"系统通知"`                    // 标题（模糊）
	Type      *int8  `form:"type" example:"1"`                        // 类型（1=系统消息 2=通知 3=私信）
	IsRead    *int8  `form:"isRead" example:"0"`                      // 是否已读（0=未读 1=已读）
	Direction *int8  `form:"direction" example:"0"`                   // 消息范围（0=全部 1=我收到的 2=我发送的）
	BeginTime string `form:"beginTime" example:"2026-01-01 00:00:00"` // 发送时间起
	EndTime   string `form:"endTime" example:"2026-12-31 23:59:59"`   // 发送时间止
	dto.PageRequest
}
